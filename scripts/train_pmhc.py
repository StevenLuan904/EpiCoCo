...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/train/train_pmhc.yml')
    ...
    # Transforms
    featurizer = FeaturizeMol(config.chem.atomic_numbers,
    ...
    logger.info(f'Num of parameters is {num_params}')
    logger.info(f'Num of trainable parameters is {num_trainable_params}')
    ...
    def train(it):
        ...
        pos_noise = pos_noise * (batch.entity == 1).unsqueeze(-1)
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=config.train.use_amp):
            loss_dict = model.get_loss(node_type=batch.node_type,
                                       ...
                                       entity=batch.entity,
                                       halfedge_entity=batch.halfedge_entity,
                                       ic50=batch.ic50,
                                       esm=batch.esm,
        ...
        log_info = '[Train] Iter %d | ' % it + ' | '.join(['%s: %.6f' % (k, v.item()) for k, v in loss_dict.items()])
