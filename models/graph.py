...
class NodeBlock(Module):
    ...
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate, residue_mode='highway'):
        super().__init__()
        ...
        if self.residue_mode == 'highway':
            self.highway_lin = Linear(node_dim, node_dim)
            # 将 bias 初始化为负数，使初始状态倾向于保留原始特征
            nn.init.constant_(self.highway_lin.bias, -3.0)
    ...
    def forward(self, x, edge_index, edge_attr, node_time):
        ...
        if self.residue_mode == 'highway':
            # 计算门控值 g (0~1)
            highway_gate = torch.sigmoid(self.highway_lin(x))
            # 外部逻辑是 h = h + res，所以这里返回 g * (out - x)
            # 最终效果: h_new = h + g*(out-h) = (1-g)*h + g*out
            return highway_gate * (out - x)
        else:
            return out
...
class NodeEncoder(Module):
    ...
    def __init__(self,
        ...
        for _ in range(num_blocks):
            block = NodeBlock(node_dim=node_dim,
                              edge_dim=edge_dim,
                              key_dim=key_dim,
                              num_heads=num_heads,
                              use_atten=use_atten,
...
class BondBlock(Module):
    ...
    def forward(self, h_bond, bond_index, h_node, atten_index=None):
        ...
        h_bond = (msg_bond_left + msg_bond_right + self.node_ffn_left(h_node[left_node]) +
                  self.node_ffn_right(h_node[right_node]) + self.self_ffn(h_bond))
...
class NodeEdgeNet(Module):
    ...
    def __init__(self, node_dim, edge_dim, num_blocks, cutoff, use_gate, **kwargs):
        ...
        if 'freeze_mhc_node_update' in kwargs and kwargs['freeze_mhc_node_update']:
            self.freeze_mhc_node_update = True
        else:
            self.freeze_mhc_node_update = False  # default not freeze mhc node

        if 'freeze_mhc_pos_update' in kwargs and kwargs['freeze_mhc_pos_update']:
            self.freeze_mhc_pos_update = True
        else:
            self.freeze_mhc_pos_update = False  # default not freeze mhc pos

        if 'residue_mode' in kwargs and kwargs['residue_mode'] is not None:
            self.residue_mode = kwargs['residue_mode']
        else:
            self.residue_mode = 'highway'
        ...
        for _ in range(num_blocks):
            self.node_blocks_with_edge.append(
                NodeBlock(
                    ...
                    hidden_dim=node_dim,
                    use_gate=use_gate,
                    residue_mode=self.residue_mode,
                ))
    ...
    def forward(self, h_node, pos_node, h_edge, edge_index, node_time, edge_time, entity=None, halfedge_entity=None):
        edge_entity = None
        if halfedge_entity is not None:
            edge_entity = torch.cat([halfedge_entity, halfedge_entity], dim=0)
        for i in range(self.num_blocks):
            # edge fetures before each block
            # 理论上，因为MHC的pos不变，所以MHC-MHC边的距离也不变，不需要mask
            ...
            # 只允许 pep-mhc界面边 和 peptide内部边 更新
            h_node_with_edge = self.node_blocks_with_edge[i](h_node, edge_index, h_edge, node_time)
            if self.update_edge:
                edge_update = self.edge_blocks[i](h_edge, edge_index, h_node, edge_time)
                if edge_entity is not None:
                    edge_update = edge_update * (edge_entity != 0).float().unsqueeze(-1)
                h_edge = h_edge + edge_update
            if self.freeze_mhc_node_update and entity is not None:
                mhc_h_node = h_node[entity == 0].clone()
            h_node = h_node + h_node_with_edge
            if self.freeze_mhc_node_update and entity is not None:
                h_node[entity == 0] = mhc_h_node

            # pos updates
            if self.update_pos:
                if self.freeze_mhc_pos_update and entity is not None:
                    mhc_pos = pos_node[entity == 0].clone()
                pos_node = pos_node + self.pos_blocks[i](h_node, h_edge, edge_index, relative_vec, distance, edge_time)
                if self.freeze_mhc_pos_update and entity is not None:
                    pos_node[entity == 0] = mhc_pos
