...
class ContigousTransition(nn.Module):
    ...
    def add_noise(self, x, time_step, batch, entity=None):
        ...
        if entity is not None:
            # Only add noise to peptide nodes (entity!=0), keep MHC nodes unchanged
            # For edges, add noise to both pep-pep and pep-MHC edges, keep MHC-MHC edges unchanged
            peptide_mask = self.mask_fn(entity).unsqueeze(-1)
            pert_peptide = a_bar.sqrt() * x + (1 - a_bar).sqrt() * noise
            pert = torch.where(peptide_mask, pert_peptide, x)
        else:
    ...
    def noise_to_step_s(self, x_t, time_t, time_s, batch):
        """
        Adds noise to an intermediate state x_t (at timestep t) to get x_s (at timestep s).
        Assumes s >= t.
        Args:
            x_t (Tensor): The tensor at timestep t. Shape [batch_size, ...].
                          Should be the continuous representation (potentially scaled one-hot).
            time_t (Tensor): A tensor of timesteps t corresponding to each item in x_t. Shape [num_timesteps], where num_timesteps >= max(batch).
            time_s (Tensor): A tensor of timesteps s corresponding to each item in x_t. Shape [num_timesteps], where num_timesteps >= max(batch).
                             It's expected that time_s[i] >= time_t[i] for all i in batch.
            batch (Tensor): A tensor of batch indices indicating which items in x_t, time_t, time_s to process. Shape [batch_size].

        Returns:
            Tensor: The noised tensor x_s at timestep s.
        """
        assert torch.all(time_s.index_select(0, batch) >= time_t.index_select(0, batch)), "Target timestep s must be >= current timestep t"

        # Extract alpha_bar_t and alpha_bar_s for the specific batch items
        a_bar_t = self.alphas_bar.index_select(0, time_t)
        a_bar_t = a_bar_t.index_select(0, batch).unsqueeze(-1)

        a_bar_s = self.alphas_bar.index_select(0, time_s)
        a_bar_s = a_bar_s.index_select(0, batch).unsqueeze(-1)

        # Calculate alpha_bar_s / alpha_bar_t ratio
        # Add a small epsilon to prevent division by zero, although alpha_bar should always be > 0
        alpha_ratio = a_bar_s / (a_bar_t + 1e-9)

        # Calculate mean and standard deviation according to the formula:
        # x_s = sqrt(alpha_bar_s / alpha_bar_t) * x_t + sqrt(1 - alpha_bar_s / alpha_bar_t) * noise
        mean_scale = alpha_ratio.sqrt()
        noise_std = (1.0 - alpha_ratio).sqrt()

        # Generate noise
        noise = torch.randn_like(x_t)

        # Calculate x_s
        x_s = mean_scale * x_t + noise_std * noise
