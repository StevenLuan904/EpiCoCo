...
class FeaturizeMol(object):
    ...
    def __init__(self, atomic_numbers, mol_bond_types, use_mask_node, use_mask_edge):
        ...
        # 这里bond_type去除了之后，还可以用halfedge_type
        self.exclude_keys = [
            'orig_keys', 'pos_all_confs', 'smiles', 'num_confs', 'i_conf_list'
    ...
    def __call__(self, data: Drug3DData):
        ...
        halfedge_entity = ((entity_mask[halfedge_index[0]] & entity_mask[halfedge_index[1]]).long() * 1) + (
            (entity_mask[halfedge_index[0]] ^ entity_mask[halfedge_index[1]]).long() * 2)

        data.halfedge_index = halfedge_index
        data.halfedge_type = halfedge_type
        data.halfedge_entity = halfedge_entity
...
def make_data_placeholder(loader, device):
    batch = next(iter(loader))

    # Ensure batch is moved to the correct device if it supports `.to()`
    if hasattr(batch, 'to'):
        batch = batch.to(device)

    # Extract relevant attributes from the batch
    attributes = [
        'allele_name', 'node_type_batch', 'halfedge_entity', 'halfedge_index', 'halfedge_type', 'halfedge_type_batch',
        'entity', 'node_type', 'node_pos'
    ]
    batch_holder = {}
    for attr in attributes:
        if hasattr(batch, attr):
            value = getattr(batch, attr)
            # Move tensors to the device if applicable
            batch_holder[attr] = value.to(device) if hasattr(value, 'to') else value
        else:
            batch_holder[attr] = None

    # Calculate additional properties
    if 'node_type_batch' in batch_holder and batch_holder['node_type_batch'] is not None:
        batch_holder['actual_n_graphs'] = torch.bincount(batch_holder['node_type_batch']).size(0)
    else:
        batch_holder['actual_n_graphs'] = 0
